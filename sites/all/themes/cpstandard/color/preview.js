...
(function ($) {
  Drupal.color = {
    ...
    callback: function(context, settings, form, farb, height, width) {
      ...
      // Text preview.
      $('#preview #preview-node .preview-content', form).css('color', $('#palette input[name="palette[text]"]', form).val());
      $('#preview #preview-content a', form).css('color', $('#palette input[name="palette[link]"]', form).val());
      $('#preview #preview-main-menu a', form).css('background-color', $('#palette input[name="palette[link]"]', form).val());
      $('#preview #preview-content', form).css('border-color', $('#palette input[name="palette[link]"]', form).val());

      // Sidebar block.
      $('#preview #preview-sidebar .preview-block', form).css('background-color', $('#palette input[name="palette[link]"]', form).val());
      $('#preview #preview-sidebar .preview-block-alt', form).css('background-color', $('#palette input[name="palette[brand-alt]"]', form).val());
      $('#preview #preview-sidebar .preview-block-alt-2,#preview #preview-sidebar .preview-block-alt-2 h2', form).css('color', $('#palette input[name="palette[link]"]', form).val());
